...
def detect_intent(text: str) -> dict:
    ...
    if any(kw in text_lower for kw in bg_keywords):
        ...
        if video_ref:
            return {
                "skill": "fal-edit-video",
                "args": [video_ref, "--model veed/video-background-removal"]
            }
        image_ref = extract_image_reference(text)
        return {
            "skill": "fal-remove-bg",
    ...
    # Audio generation (TTS) (Priority 3)
    tts_keywords = ["text to speech", "tts", "speak", "say", "voice", "narrate", "read aloud"]
    if any(kw in text_lower for kw in tts_keywords):
        return {
            "skill": "fal-generate-audio",
            "args": [text]
        }

    # Music generation (Priority 4)
    music_keywords = ["music", "song", "soundtrack", "bgm", "beat", "sound effect", "sfx"]
    if any(kw in text_lower for kw in music_keywords) and "video" not in text_lower:
        return {
            "skill": "fal-generate-music",
            "args": [text]
        }

    # Transcription / speech-to-text (Priority 5)
    transcribe_keywords = ["transcribe", "speech to text", "stt", "transcription", "caption"]
    if any(kw in text_lower for kw in transcribe_keywords):
        audio_ref = extract_audio_reference(text)
        if audio_ref:
            return {
                "skill": "fal-transcribe",
                "args": [audio_ref]
            }

    # Avatar / lipsync (Priority 6)
    avatar_keywords = ["avatar", "lipsync", "talking head", "speaking video", "animated face"]
    if any(kw in text_lower for kw in avatar_keywords):
        audio_ref = extract_audio_reference(text)
        image_ref = extract_image_reference(text)
        video_ref = extract_video_reference(text)
        args = []
        if audio_ref:
            args.append(f"--audio-url {audio_ref}")
        if image_ref:
            args.append(f"--image-url {image_ref}")
        if video_ref:
            args.append(f"--video-url {video_ref}")
        if args:
            return {
                "skill": "fal-avatar",
                "args": args
    ...
    # Video editing (Priority 7)
    video_edit_keywords = ["remove background", "remix", "video edit", "apply effect", "stylize video", "video effect"]
    if any(kw in text_lower for kw in video_edit_keywords):
        video_ref = extract_video_reference(text)
        args = []
        if video_ref:
            args.append(video_ref)
        if "remove background" in text_lower:
            args.append("--model veed/video-background-removal")
        elif "remix" in text_lower:
            args.append("--model fal-ai/sora-2/video-to-video/remix")
        else:
            args.append("--model fal-ai/wan-effects")
        return {
            "skill": "fal-edit-video",
            "args": args
        }

    # Video generation (Priority 8 for video keywords)
    ...
    if any(kw in text_lower for kw in video_keywords):
        # Check if image-to-video, video-to-video, or text-to-video
        image_ref = extract_image_reference(text)
        video_ref = extract_video_reference(text)
        ...
        if video_ref:
            return {
                "skill": "fal-generate-video",
                "args": [text, f"--video-url {video_ref}"]
            }
        return {
            "skill": "fal-generate-video",
...
def extract_audio_reference(text: str) -> str:
    """
    Extract audio URL or path from text

    Args:
        text: Text containing audio reference

    Returns:
        Audio URL or file path, or None if not found
    """
    audio_url_match = re.search(r'https?://[^\s]+\.(mp3|wav|m4a|ogg|webm)', text, re.I)
    if audio_url_match:
        return audio_url_match.group(0)

    audio_path_match = re.search(r'[\w./~-]+\.(mp3|wav|m4a|ogg|webm)', text, re.I)
    if audio_path_match:
        return audio_path_match.group(0)
